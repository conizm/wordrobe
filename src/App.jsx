...
const fetchJishoData = async (term) => {
    try {
        ...
        const finalUrl = `${myGasProxy}?url=${encodeURIComponent(targetUrl)}`;
        const res = await fetchWithTimeout(finalUrl, {}, 4000);
        if (!res.ok) return null;
...
const fetchRichData = async (term) => {
    ...
    let searchWord = term, isJpSearch = isJapanese(term), tips = "Jisho.org / Google", primaryTrans = "", meanings = [], pronunciation = "", synonyms = [], audioUrl = "", jishoFound = false;

    try {
        const jishoData = await fetchJishoData(term);
        if (jishoData && jishoData.data && jishoData.data.length > 0) {
            jishoFound = true;
            tips = "Jisho.org / Google";
            if (isJpSearch) {
...
const FolderSelectModal = ({ isOpen, onClose, folders, onSelectFolder, onCreateFolder }) => {
    ...
    return (
        <div className="fixed inset-0 z-[90] flex items-end justify-center">
            ...
            <div 
                className="relative bg-[#F8FAFC] w-full max-w-md rounded-t-3xl p-4 shadow-2xl slide-up border-t border-white/50 pb-8"
                    ...
                    {folders.map(folder => (
                        <button key={folder} onClick={() => onSelectFolder(folder)} className="w-full text-left p-4 rounded-xl neu-flat hover:bg-gray-50 flex justify-between items-center transition-transform active:scale-[0.99]">
                            <span className="font-bold text-gray-700 flex items-center">
                                {folder}
                                {folder === 'main' && <span className="text-xs text-gray-400 bg-gray-100 px-2 py-1 rounded ml-2">Default</span>}
                            </span>
...
const DialogModal = ({ isOpen, type, message, onConfirm, onCancel }) => {
    if (!isOpen) return null;
    return (
        <div className="fixed inset-0 bg-slate-100/90 z-[110] flex items-center justify-center p-4 backdrop-blur-sm fade-in">
...
const SplashScreen = () => {
    return (
        <div className="fixed inset-0 bg-[#F8FAFC] z-[120] flex items-center justify-center">
            <div className="w-32 h-32 md:w-48 md:h-48 logo-anim">
                <img src="/images/wordrobe-welcome.png" alt="Splash Logo" className="w-full h-full object-contain drop-shadow-md" />
            </div>
        </div>
    );
};
...
const WelcomeModal = ({ isOpen, onClose }) => {
    if (!isOpen) return null;
    const tosUrl = "https://maze-dracorex-e48.notion.site/wordrobe-Terms-of-Use-2c404d09b6cc803bad7bd012f0deed88?pvs=73"; 

    return (
        <div className="fixed inset-0 bg-[#F8FAFC] z-[105] flex items-center justify-center p-4 backdrop-blur-sm fade-in">
            <div className="neu-flat bg-[#F8FAFC] p-5 w-full max-w-sm text-center border border-white relative overflow-hidden flex flex-col max-h-full">
                 
                <div className="relative z-10 flex flex-col h-full justify-center">
                     
                    <div className="mb-4 flex justify-center">
                        <img src="/images/welcome-logo.png" alt="Welcome to wordrobe" className="h-8 w-auto" /> 
                    </div>

                    <div className="space-y-2 text-left mb-4 px-1">
                        <div className="flex items-center gap-3 bg-white/60 p-2 rounded-lg border border-white/50 shadow-sm">
                            <div className="w-8 h-8 rounded-lg bg-[#258FAF]/10 flex items-center justify-center text-[#258FAF] shrink-0">
                                <i className="fas fa-search text-xs"></i>
                            </div>
                            <div>
                                <p className="font-bold text-sm text-gray-700">検索 & 保存</p>
                                <p className="text-xs text-gray-400 leading-tight">日英の単語 / フレーズ検索・保存。</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 bg-white/60 p-2 rounded-lg border border-white/50 shadow-sm">
                            <div className="w-8 h-8 rounded-lg bg-[#258FAF]/10 flex items-center justify-center text-[#258FAF] shrink-0">
                            ...
                            <div>
                                <p className="font-bold text-sm text-gray-700">復習モード</p>
                                <p className="text-xs text-gray-400 leading-tight">カード形式でサクサク復習。</p>
                            </div>
                        </div>
                        <div className="flex items-center gap-3 bg-white/60 p-2 rounded-lg border border-white/50 shadow-sm">
                            <div className="w-8 h-8 rounded-lg bg-[#258FAF]/10 flex items-center justify-center text-[#258FAF] shrink-0">
                            ...
                            <div>
                                <p className="font-bold text-sm text-gray-700">AI解説</p>
                                <p className="text-xs text-gray-400 leading-tight">APIキー設定でAI機能追加</p>
                            </div>
                        </div>
                    </div>

                    <div className="mb-1 bg-[#258FAF]/5 border border-[#258FAF]/20 rounded-lg p-3 flex items-center justify-center gap-2 text-center">
                        <div className="text-[#258FAF] text-sm">
                            <i className="fas fa-mobile-alt"></i>
                        </div>
                        <p className="text-xs text-[#258FAF] font-bold leading-tight">
                            ブラウザ「ホーム画面に追加」で全画面表示
                        </p>
                    </div>

                    <div className="mt-1 border-t border-gray-100 pt-3">
                        <p className="text-[11px] text-gray-400 mb-5 leading-relaxed text-center">
                            <a href={tosUrl} target="_blank" rel="noopener noreferrer" className="text-[#258FAF] underline hover:text-[#1d6f8c] font-bold">利用規約</a>
                            を確認・同意の上、ご利用ください。
                        </p>
                        <button 
                            onClick={onClose} 
                            className="bg-[#258FAF] text-white w-full py-3 rounded-xl font-bold text-sm shadow-lg active:scale-95 transition-transform"
                        >
                            利用規約に同意してはじめる
                        </button>
                    </div>
...
const SettingsModal = ({ isOpen, onClose, onExport, onImport, user, onLogin, onLogout, openAlert, openConfirm, onRequestNotification }) => {
    const [apiKey, setApiKey] = useState('');
    const [isSaved, setIsSaved] = useState(false);
    const [testStatus, setTestStatus] = useState(null);
    const [currentPermission, setCurrentPermission] = useState(('Notification' in window) ? Notification.permission : 'default');
    ...
    const handleForceDownload = async () => {
        if (!user) return openAlert("ログインしてください");
        if (!confirm("クラウド上のデータで、今のデータを上書きしてもよろしいですか？")) return;
        ...
        try {
            ...
            if (doc.exists) {
                ...
                if (cloudWords.length === 0) {
                    return openAlert("クラウドにデータがありませんでした");
                }

                await db.words.clear();
                await db.words.bulkAdd(cloudWords);
                 
                alert(`復元成功！\n${cloudWords.length}語をダウンロードしました。\n画面を更新します。`);
                window.location.reload();
            } else {
                openAlert("クラウドにデータが見つかりませんでした");
            }
        } catch (e) {
            console.error(e);
    ...
    const handleManualSync = async () => {
        ...
        try {
            ...
            await db_firebase.collection('users').doc(user.uid).set({
                words: words, 
            ...
            console.error(e);
            openAlert(`保存エラー: ${e.message}`);
        }
    };

    const handleReqNotif = () => {
        if (!user) {
            openAlert("通知機能を使うにはGoogleログインが必要です");
            return;
        }
        onRequestNotification().then(() => {
            if ('Notification' in window) {
                setCurrentPermission(Notification.permission);
            }
        });
            ...
            <div className="neu-flat bg-[#F8FAFC] p-6 w-full max-w-md max-h-[90vh] overflow-y-auto border border-white">
                            ...
                            <div className="neu-pressed bg-[#258FAF]/5 p-4 rounded-xl border border-[#258FAF]/20 relative overflow-hidden text-center">
                                    ...
                                    <div className="flex gap-2 justify-center mb-3">
                                        <button 
                                            ...
                                            className="bg-[#258FAF] text-white px-3 py-2 rounded-lg text-xs font-bold shadow-md flex items-center justify-center gap-1"
                                        >
                                            <i className="fas fa-upload"></i> 保存
                                        </button>
                                        <button 
                                            onClick={handleForceDownload} 
                                            className="bg-white text-[#258FAF] border border-[#258FAF] px-3 py-2 rounded-lg text-xs font-bold shadow-sm flex items-center justify-center gap-1"
                                        >
                                            <i className="fas fa-download"></i> 受信
                                        </button>
                                    </div>
                ...
                <div className="mt-8 flex flex-col items-center gap-4">
                    <div className="flex gap-3 w-full">
                        ...
                        <button 
                            onClick={handleReqNotif} 
                            className={`neu-btn w-12 h-12 flex items-center justify-center rounded-xl transition-colors duration-300 ${currentPermission === 'granted' ? 'text-[#475569]' : 'text-gray-300'}`}
                            title={currentPermission === 'granted' ? "通知はONです" : "通知をONにする"}
                        >
                            <i className={`${currentPermission === 'granted' ? 'fas fa-bell' : 'fas fa-bell-slash'} text-2xl`}></i>
                        </button>
                    </div>
                    ...
                    <div className="flex flex-col items-center gap-1">
                        ...
                        <p className="text-[10px] text-gray-300 font-mono">
                            &copy; 2025 <a href="https://www.instagram.com/conizm_/" target="_blank" rel="noopener noreferrer" className="hover:text-gray-500 transition-colors">conizm_</a> <a href="https://maze-dracorex-e48.notion.site/2c504d09b6cc806a8bdbeb3b6d2438f6?pvs=73" target="_blank" rel="noopener noreferrer" className="hover:text-gray-500 transition-colors">v.1.3.1</a>
                        </p>
                    </div>
                </div>
            </div>
...
const SearchView = ({ onSaveWord, savedWords, setToast, externalResult, setExternalResult, isFromList, onBack, openAlert, onUpdateWord }) => {
    ...
    return (
        <div className="fade-in">
            ...
            {!loading && !currentData && <div className="text-center py-20 opacity-40 select-none"><div className="neu-flat w-32 h-32 rounded-full flex items-center justify-center mx-auto mb-6 text-[#258FAF]" style={{borderRadius: '50%'}}><img src="/images/wordrobe-lptop.png" className="w-20 h-20 opacity-60 object-contain" alt="Search" /></div><p className="font-bold text-gray-500"></p></div>}
            ...
            {currentData && (
                <div ref={resultRef} className="neu-flat p-5 md:p-6 mb-8 fade-in relative mt-8 allow-select">
                    ...
                    <div className="flex justify-between items-start mb-6 border-b border-gray-100 pb-4">
                        ...
                        <div className="flex gap-3">
                            <button onClick={() => speak(currentData.word, currentData.audioUrl)} className="neu-btn w-10 h-10 flex items-center justify-center rounded-full text-[#258FAF]" style={{borderRadius: '50%'}}><i className="fas fa-volume-up"></i></button>
                            <button onClick={() => onSaveWord(currentData)} className={`neu-btn w-10 h-10 flex items-center justify-center rounded-full ${isSaved ? 'text-yellow-500 neu-pressed' : 'text-gray-400'}`} style={{borderRadius: '50%'}}><i className="fas fa-bookmark"></i></button>
                        </div>
                    </div>
    ...
    return (
        <div onClick={handleClick} className={`neu-flat p-4 flex justify-between items-center cursor-pointer active:scale-[0.99] transition ${isMem ? 'opacity-40 grayscale bg-gray-100' : ''}`}>
            ...
            <div className="flex gap-2 items-center">
                <button className="neu-btn w-10 h-10 flex items-center justify-center rounded-full text-[#258FAF] shrink-0 relative z-10" onClick={(e) => { e.stopPropagation(); speak(item.word, item.audioUrl); }} style={{borderRadius: '50%'}}><i className="fas fa-volume-up"></i></button>
                <button className="neu-btn w-10 h-10 flex items-center justify-center rounded-full text-gray-400 hover:text-red-400 shrink-0 relative z-10" onClick={(e) => { e.stopPropagation(); onDelete(item.word); }} style={{borderRadius: '50%'}}><i className="fas fa-trash"></i></button>
            </div>
        </div>
    );
};
...
const ReviewView = ({ savedWords, folderList, onFinish, openAlert }) => {
    ...
    const handleNext = (isClear) => {
        ...
        setTimeout(() => {
            setIsFlipped(false);
            if (index + 1 >= queue.length && isClear) {
                 setIsStarted(false);
                 setQueue([]);
                 setIndex(0);
    ...
    if (!isStarted) {
        return (
            <div className="h-full flex flex-col justify-center items-center text-center fade-in">
                ...
                <div className="neu-flat w-40 h-40 rounded-full flex items-center justify-center mb-8 text-[#258FAF]" style={{borderRadius: '50%'}}>
                ...
                <div className="relative mb-4 w-72 z-20">
                    <button 
    ...
    if (!card) return null;
    const questionText = testMode === 'en_ja' ? card.word : card.translation;
    const answerMain = testMode === 'en_ja' ? card.translation : card.word;
                ...
                <div className="card-inner">
                    <div className="card-front neu-flat flex flex-col items-center justify-center p-8 text-center bg-[#F8FAFC] relative">
                        ...
                        <div className="flex-1 flex flex-col justify-center items-center w-full">
                            ...
                            {testMode === 'en_ja' && (
                                <button onClick={(e) => { e.stopPropagation(); speak(card.word, card.audioUrl); }} className="neu-btn w-12 h-12 flex items-center justify-center rounded-full text-[#258FAF] mt-8" style={{borderRadius: '50%'}}>
                    ...
                    <div className="card-back neu-flat flex flex-col items-center justify-center p-8 text-center bg-gradient-to-b from-[#F8FAFC] to-white">
                        ...
                        <div className="w-full overflow-y-auto max-h-32 text-sm text-gray-500 mb-4 px-4 space-y-2">
                            {card.meanings && card.meanings.length > 0 ? card.meanings.map((m, i) => <p key={i}><span className="font-bold">{m.partOfSpeech}:</span> {m.definitions[0].ja}</p>) : <p>{card.definition}</p>}
                        </div>
                        <button onClick={(e) => { e.stopPropagation(); speak(card.word, card.audioUrl); }} className="neu-btn w-12 h-12 flex items-center justify-center rounded-full mt-2 text-[#258FAF]" style={{borderRadius: '50%'}}>
                            <i className="fas fa-volume-up"></i>
                        </button>
                    </div>
                </div>
...
export default function App() {
    ...
    const [showSplash, setShowSplash] = useState(localStorage.getItem('is_first_launch_v1') !== 'false');
    const [isFirstLaunch, setIsFirstLaunch] = useState(localStorage.getItem('is_first_launch_v1') !== 'false');
    ...
    const [dialog, setDialog] = useState({ isOpen: false, type: 'alert', message: '' });
    ...
    useEffect(() => {
        db.words.toArray().then(w => setSavedWords(w.sort((a,b) => (b.addedAt || 0) - (a.addedAt || 0))));
        const f = localStorage.getItem('aitan_folders');
        if (f) setFolders(JSON.parse(f));
        if (showSplash) setTimeout(() => setShowSplash(false), 2500);
    ...
    const handleLogin = () => {
        ...
        auth.signInWithPopup(provider).catch(e => { console.error(e); openAlert(`ログインエラー: ${e.message}`); });
    };

    const handleLogout = () => {
        if(!auth) return;
        auth.signOut().then(() => { setToastMsg('ログアウトしました'); setUser(null); });
    };
    ...
    const handleChangeFolder = async (folderName) => {
        ...
        try {
            ...
            if (wordInDb) await db.words.update(wordInDb.id, { folder: folderName });
        } catch (e) {}
    ...
    const handleForceDownload = async () => {
        ...
        try {
            ...
            if (doc.exists) {
                const cloudWords = doc.data().words || [];
                if (cloudWords.length === 0) return openAlert("クラウドにデータがありませんでした");
                await db.words.clear();
                await db.words.bulkAdd(cloudWords);
                alert(`復元成功！\n${cloudWords.length}語をダウンロードしました。\n画面を更新します。`);
                window.location.reload();
    ...
    return (
        <div className="h-screen-safe">
            ...
            <WelcomeModal isOpen={isFirstLaunch && !showSplash} onClose={handleCompleteOnboarding} />
             
            <header className="custom-header neu-flat">
                <div className="flex-1 flex justify-center items-center" onClick={resetToHome}>
                    <img src="/images/wordrobe-top.png" alt="wordrobe" className="h-8 md:h-12" />
                </div>
                <button onClick={() => setIsSettingsOpen(true)} className="absolute right-5 md:right-10 text-gray-400 hover:text-[#258FAF] transition-colors"><i className="fas fa-cog"></i></button>
            </header>
            ...
            <nav className={`custom-nav neu-flat nav-bottom ${uiDisabledClass}`}>
                <button onClick={resetToHome} className="nav-btn flex flex-col items-center w-1/3">
                    <div className={`w-10 h-10 flex items-center justify-center rounded-xl mb-1 ${tab === 'search' ? 'neu-pressed' : ''}`}>
                        <i className="fas fa-search text-lg"></i>
                    </div>
                    <span className="text-[10px] font-bold">検索</span>
                </button>
                <button onClick={() => setTab('review')} className="nav-btn flex flex-col items-center w-1/3">
                    <div className={`w-10 h-10 flex items-center justify-center rounded-xl mb-1 ${tab === 'review' ? 'neu-pressed' : ''}`}>
                        <svg width="22" height="22" viewBox="0 0 24 24" fill="currentColor">
                            <g transform="rotate(-45 12 12)">
                                <path d="M9 2H13L17 6V22H5V6L9 2ZM11 7.5C12.38 7.5 13.5 6.38 13.5 5C13.5 3.62 12.38 2.5 11 2.5C9.62 2.5 8.5 3.62 8.5 5C8.5 6.38 9.62 7.5 11 7.5Z" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
                            </g>
                        </svg>
                    </div>
                    <span className="text-[10px] font-bold">復習</span>
                </button>
                <button onClick={() => setTab('list')} className="nav-btn flex flex-col items-center w-1/3">
                    <div className={`w-10 h-10 flex items-center justify-center rounded-xl mb-1 ${tab === 'list' ? 'neu-pressed' : ''}`}>
                        <svg width="26" height="26" viewBox="0 0 24 24" fill="currentColor">
                            <path d="M6 2H11V22H6C4.89543 22 4 21.1046 4 20V4C4 2.89543 4.89543 2 6 2ZM13 2H18C19.1046 2 20 2.89543 20 4V20C20 21.1046 19.1046 22 18 22H13V2ZM9 11C8.44772 11 8 11.4477 8 12C8 12.5523 8.44772 13 9 13C9.55228 13 10 12.5523 10 12C10 11.4477 9.55228 11 9 11ZM15 11C14.4477 11 14 11.4477 14 12C14 12.5523 14.4477 13 15 13C15.5523 13 16 12.5523 16 12C16 11.4477 15.5523 11 15 11Z" />
                        </svg>
                    </div>
